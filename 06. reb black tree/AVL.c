...
struct AVLNode * Tree_Delete(AVLTree root, struct AVLNode * x) //从以 root 为根的 AVL 中删除结点 x
{
	if (root == NULL || x == NULL)
		return NULL;

	if (x->key < root->key)        // 待删除的节点在"root的左子树"中
	{
	...
	else if (x->key > root->key)// 待删除的节点在"root的右子树"中
	{
